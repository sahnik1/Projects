...
evalDProp i1 i2 = (eval (Map.fromList[("x", 2.0)]) (Mult (Add (Const i1) (Const i2)) (Var "x"))) - ((i1+i2)*(2.0)) <= 1.0 
...
evalFProp i1 i2 = (eval (Map.fromList[("x", 2.0)]) (Mult (Add (Const i1) (Const i2)) (Var "x"))) - ((i1+i2)*(2.0)) <= 1.0
...
pdDProp d1 = 
    ...
    in (eval listd expression) - ((6.0)*(d1**2.0)) <= 1.0
...
pdFProp d1 = 
    ...
    in (eval listd expression) - ((8.0)*(d1**3.0)) <= 1.0
...
simpDProp d1 d2 =
    ...
    in (eval empty (simplify empty presimp)) - ((d1*d2)**2) <= 1.0
